...
bool Server::getGps() {
    ...
    if (osrm->haversine(last_gps.second, last_gps.first, cur_gps.second, cur_gps.first) > DISTANCE_THRESHOLD)
        return false;
    
    cur_gps.first += 0.00001f;
    return true;
}
...
int Server::SendClient(const ServerData& data) {
    ssize_t len = -1;
    try {
        send(client_fd, &data, sizeof(data), MSG_NOSIGNAL);
    }
    catch (...) {
        return -1;
    }
...
int Server::run() {
    ...
    // 경로 탐색 알고리즘
    server_data.speed = speed;
    // test
    is_navigating = true;
    if (is_navigating) {
        double bearing = 0.;
        if (speed > BEARING_THRESHOLD) 
            bearing = calculateBearing(last_gps.first, last_gps.second, cur_gps.first, cur_gps.second);
        //bearing = 0.0;
        //cur_gps = {37.2940, 127.0198}; // 예시 GPS 좌표, 실제 GPS 데이터로 대체 필요
